...
for line in f_vcf.readlines():
	if line.startswith('#'):
		...
		change_start=int(record[1])+1
		change_end=int(record[7].split(';')[0].split('=')[1])
		if change_end==change_start:
			chr_pos=record[0]+":"+str(change_start)
		elif change_end>change_start:
			chr_pos=record[0]+":"+str(change_start)+"-"+str(change_end)
		ref=record[3]
		alt=record[4]
...
data_filter=del_fasta_dd[del_fasta_dd["peptide_length"]>=11]
