...
import pandas as pd
import sys,getopt
opts,args=getopt.getopt(sys.argv[1:],"hi:c:o:s:",["input_ins_vep_file","input_ins_vcf_file","out_dir","sample_id"])
input_ins_vep_file =""
input_ins_vcf_file=""
out_dir=""
sample_id=""
USAGE='''
	...
	usage: python insertion2fasta.py -i <input_vep_file> -o <outdir> -s <sample_id>
		required argument:
			...
			-c | --input_ins_vcf_file: input vcf file
...
for opt,value in opts:
	...
	elif opt in ("-c","--input_ins_vcf_file"):
		input_ins_vcf_file=value
...
#print coverage
if (input_ins_vep_file =="" or input_ins_vcf_file=="" or out_dir =="" or sample_id==""):
	print USAGE
	sys.exit(2)	
...
ref_sv_vcf={}
alt_sv_vcf={}
f_vcf=open(input_ins_vcf_file,'r')
for line in f_vcf.readlines():
	if line.startswith('#'):
		pass
	else:
		record=line.strip().split('\t')
		insert_start=int(record[1])
		insert_end=insert_start+1
		chr_pos=record[0]+":"+str(insert_start)+"-"+str(insert_end)
		ref=record[3]
		alt=record[4]
		ref_sv_vcf[chr_pos]=ref
		alt_sv_vcf[chr_pos]=alt
...
#allele_alt=[]
#for ele in location:
#	allele_ref.append(ref_sv_vcf[ele])
#	allele_alt.append(alt_sv_vcf[ele])
gene_symbol=[]
strand=[]
for line in extra:
	element=line.split(';')
...
for i in range(len(location)):
	if transcript_seq[i]=="NULL":
		...
		wt_head='>WT_'+gene_symbol[i]
		mt_head='>MT_'+gene_symbol[i]
...
#data_filter=ins_fasta_dd[(ins_fasta_dd["mutation_peptide_length"]>=11) & (ins_fasta_dd["wild_peptide_length"]>=11)]
